<h1 align="center">Frontend 개발자 이동근의 포트폴리오 페이지입니다</h1>
...
+ Frontend Skeleton 코드 제작
+ Viewport에 따른 웹앱 화면 구성
+ 첫 이용 고객을 위한 Onsikgo 사용 설명 페이지 제작
+ 고객과 점주의 상태값에 따른 화면 구성 및 알림페이지 구성
+ 상품 및 재고 CRUD 페이지를 구성
+ WordCloud와 Chart.js 등 외부 패키지를 도입하여 Onsikgo를 통해 얻은 수익 시각화
+ 현재 위치 또는 가게 종류에 따라 현재 등록된 제품 목록 나열
...
+ Vuex를 도입하여 상태관리 시도
+ 화면 크기에 따라 다르게 보여지는 웹앱 형식으로 구성
  + 포스기 사용으로 기존 컴퓨터로 웹서비스 이용이 불편하다는 실사용자의 피드백을 통해 변경
...
+ 제품 주문, 주문확인과 같은 알림페이지의 알림이 실시간으로 반영되지 않았습니다
  + 서버와 데이터는 정확하게 전달되었으나, 웹 특성상 요청이 없는 경우 새로고침이 되지 않아 실시간으로 확인이 불가능했습니다. FireBase를 도입하여 실시간 알림 문제를 해결하였으나, 휴대폰 화면이 꺼져있는 상태(홀드) 상태에 직접 알림을 보내는 것은 불가능하였습니다.
+ 로컬에서 보여지는 화면 구성과 서버에서 보여지는 화면 구성이 상이하였습니다
  + Bootstrap과 Vuetify를 섞어 화면을 구성하였는데, 서버에 올라가며 CSS 우선순위 충돌이 일어난 것 같았습니다. 
  + 충돌 요소를 찾아 삭제하며 로컬 화면이 아닌 서버에 보여지는 화면을 기준으로 CSS를 전면 수정하여 해결하였습니다
...
+ Frontend Skeleton 코드 제작
+ 디스플레이 크기에 따른 반응형 웹 구성
+ 회원가입, 로그인, 개인 프로필 페이지 전반 구성
+ 메인 페이지 구성
+ 받아온 경기 데이터를 바탕으로 시뮬레이션 구현
...
+ 경기 예측과 실제 경기 결과를 비교하여 서비스 신뢰도 향상
...
* 방대한 양의 데이터를 불러와 특정 페이지의 로딩 속도가 느렸습니다
  ...
  * react Suspense와 삼항 연상자를 통해 빈 페이지 로딩을 방지하였고, % 표시를 통해 서비스가 작동 중임을 표시하였습니다
...
+ 메인페이지와 사용자 편의를 위한 네비게이션 구성
+ 마이페이지와 개발 성향 조사를 위한 설문조사 페이지 구성
+ 경험치를 통해 학습 수준을 한 눈에 알 수 있게 구성

<br>

 👍 시도 및 도전

+ Recoil을 도입하여 상태관리 시도
+ Three.js를 활용하여 페이지를 구성
+ 페이지 전환에 오랜 시간이 걸렸습니다
  + react Suspense와 삼항연산자를 활용하여 로딩 시간을 %로 구현하였고, CS 지식을 함께 나타내어 지루함을 줄였습니다
+ 학습을 통해 MD 파일을 작성하게 될 경우 개인 페이지에 Git의 잔디와 같이 학습량이 기록되도록 구현

<br>

✍️ 아쉬운 점

+ Three.js 학습에 너무 많은 시간을 쏟아 완성도가 아쉽습니다.
